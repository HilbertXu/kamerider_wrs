...
/*
	...
	bool is_may_catch_wrong()
	{
		...
		{
			LDistance = sqrt(pow(Avatar.lefthand[0]-myOject[i].x,2)+pow(Avatar.lefthand[1]-myObject[i].y,2));
			RDistance = sqrt(pow(Avatar.righthand[0]-myOject[i].x,2)+pow(Avatar.righthand[1]-myOject[i].y,2));
		//cout<<"---------in is_may_catch_wrong()-----------"<<endl;
		...
		switch(moveStep)
			{
				...
				*/
				...
				{
					if(isLeftMayWrong)
					{output = "Please don't pick the object near your left hand which is not the target! Our target is "+t_object+", please keep finding.";}
					if(isRightMayWrong)
					{output = "Please don't pick the object near your right hand which is not the target! Our target is "+t_object+", please keep finding.";}
					break;
				}
				default:

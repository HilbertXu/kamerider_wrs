...
string externNavCmd;//调用外部自定义导航程序的终端命令,支持-Tx与-Ty传入目标平移距离
...
ros::Publisher pub_Cup;//向Avatar发送消息
...
{//自制控制器，操纵机械臂抓取给定物体，机械臂无避障功能
	...
	if(externNavCmd.empty())
	{
		...
		{//导航太不靠谱了，第一次无意间成功后，录屏的好几十次没有一次成功(╯‵□′)╯︵┻━┻
			...
			sprintf(Buffer,"rosrun interactive_cleanup PubGoalPose %.17g %.17g 0 0 0 %.17g %.17g",
	...
	else
	{
		...
		double Ty=Gy-D;
		char Buffer[MAX_PATH];
		sprintf(Buffer,"%s -Tx %.17g -Ty %.17g",&(externNavCmd[0]),Tx,Ty);
		cout<<"运行导航命令：\n\t"<<Buffer<<endl;
		system(Buffer);
		printf("\t我认为已经到达目标位置了\n");
	}
	...
	printf("--------------------------------\n");
	printf("机械臂开环控制是一种艺术，本阁认为已经抓住物体了\n");
	printf("元芳，你怎么看？\n\n");
	
	ros::Duration(JointDurT).sleep();
	//不判断手部关节角是否合零了……一次抓不住就很难再抓住了
	printf("大人真乃神人也，卑职这就告诉Avatar已经抓住了\n\n\n");
	{
		interactive_cleanup::InteractiveCleanupMsg interactive_cleanup_msg;
		interactive_cleanup_msg.message="Object_grasped";
		pub_Cup.publish(interactive_cleanup_msg);
		exit(0);
	}
}
...
int main(int argc, char **argv)
{
	...
	
	{
		...
		for(int i=1;i<argc;i++)
		{
			...
			if(i+1<argc&&string(argv[i])=="-externNavCmd")
			{
				externNavCmd=argv[i+1];
			}
		}
	}
	
	ros::init(argc, argv, "DOFmove_AutoGcp");
	ros::NodeHandle n;
	...
	pub_Cup=n.advertise<interactive_cleanup::InteractiveCleanupMsg>("/interactive_cleanup/message/to_moderator", 1);
	task_pub = n.advertise<std_msgs::String>("/task_control",1);

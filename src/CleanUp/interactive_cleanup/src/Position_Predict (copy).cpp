...
//png, json, xml文件保存路径
string png_dir = "/home/kamerider/sim_ws/src/CleanUp/interactive-cleanup-ros/openpose_img";
string json_dir = "/home/kamerider/sim_ws/src/CleanUp/interactive-cleanup-ros/json_files";
string xml_dir = "/home/kamerider/sim_ws/src/CleanUp/interactive-cleanup-ros/UPose.xml";
...
{
	ROS_INFO("READING JSON FILES");
	ifstream Json(FullPath);
	string TmpStr;
	do
	{
		Json>>TmpStr;
		if(TmpStr=="\"pose_keypoints_2d\":[")
		{
			string DataStr;
			Json>>DataStr;
			int Nums=0;
		...
		if(TmpStr=="\"hand_left_keypoints_2d\":[")
		{
			string DataStr;
			Json>>DataStr;
			int Nums=0;
		...
		if(TmpStr=="\"hand_right_keypoints_2d\":[")
		{
			string DataStr;
			Json>>DataStr;
			int Nums=0;
...
void openposeCallback(const std_msgs::String::ConstPtr& msg)
{
	...
	if(msg->data == "finish")
	{
		string json_path_pick = json_dir + "/pick_it_up_keypoints.json";
		string json_path_clean = json_dir + "/clean_up_keypoints.json";
		ifstream json_file_pick;
		ifstream json_file_clean;
		json_file_pick.open(&(json_path_pick[0]));
		json_file_clean.open(&(json_path_clean[0]));
		while(!(json_file_pick.is_open() && json_file_clean.is_open()))
		{
			json_file_pick.close();
			json_file_clean.close();
			ROS_INFO("WAITING FOR JSON FILES......");
			cv::waitKey(1000);
			json_file_pick.open(&(json_path_pick[0]));
			json_file_clean.open(&(json_path_clean[0]));
		}
	...
	{
		sprintf(Buffer, "%s/pick_it_up_keypoints.json", &(json_dir[0]));
		vector<int>DataNums;
		vector<double>Data;
		...
		std::cout<<"1111"<<Data[MNeckNum]<<" "<<Data[LWristNum * 3]<<" "<<Data_L[IFinSecNum * 3]<<" "<<Data[RWristNum * 3]<<" "<<Data_R[IFinSecNum * 3]<<endl;

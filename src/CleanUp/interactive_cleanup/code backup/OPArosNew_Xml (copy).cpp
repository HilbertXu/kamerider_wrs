...
void DealJson()
{
	...
	string xml_1=SaveDir + "/pick_it_up_pose.xml";
	string xml_2=SaveDir + "/pick_it_up_hand_right.xml";
	string xml_3=SaveDir + "/pick_it_up_hand_left.xml";
	string xml_4=SaveDir + "/clean_up_pose.xml";
	string xml_5=SaveDir + "/clean_up_hand_right.xml";
	string xml_6=SaveDir + "/clean_up_hand_left.xml";
	...
	//现在在SaveDir文件夹下已经有了1_keypoints.json和2_keypoints.json
	{
		...
		char PoseXmlPath[MAX_PATH];sprintf(PoseXmlPath, "%s/pick_it_up_pose.xml", &(SaveDir[0]));
		char RhandXmlPath[MAX_PATH];sprintf(RhandXmlPath, "%s/pick_it_up_hand_right.xml", &(SaveDir[0]));
		char LhandXmlPath[MAX_PATH];sprintf(LhandXmlPath, "%s/pick_it_up_hand_left.xml", &(SaveDir[0]));
...

	{
		...
		vector<double>Data_R;
		//ReadJsonKeypoints(Buffer, DataNums, Data, DataNums_L, Data_L, DataNums_R, Data_R);
		char PoseXmlPath[MAX_PATH];sprintf(PoseXmlPath, "%s/clean_up_pose.xml", &(SaveDir[0]));
		char RhandXmlPath[MAX_PATH];sprintf(RhandXmlPath, "%s/clean_up_hand_right.xml", &(SaveDir[0]));
		char LhandXmlPath[MAX_PATH];sprintf(LhandXmlPath, "%s/clean_up_hand_left.xml", &(SaveDir[0]));
		ReadXmlKeypoints(PoseXmlPath,RhandXmlPath,LhandXmlPath,Data,Data_R,Data_L);
...
void imageCallback(sensor_msgs::Image msg)
{
	...
	if(PngIsSave)
	{
		sprintf(Buffer,"%s/%d.png",&(SaveDir[0]),PngStaticNum);
		imwrite(Buffer,Img);
		PngIsSave=false;
		if (PngStaticNum == 1)
		{
...
{	
	SaveFileName = argv[1];
	SaveDir = argv[2];
	ReadModelsXml(SaveFileName,M_Pos_Furniture,Furniture_Names);//读取xml文件
